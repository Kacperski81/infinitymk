 ...
 * Entry detection uses IntersectionObserver at 0.12 threshold so the
 * reveal begins just as the section peeks into the viewport.
 */
export default function AboutTheOwner({ scrollY }: { scrollY: number }) {
  const { sectionRef, isVisible, fadeOutStyle, parallaxStyle } = useScrollFade(scrollY, {
    inThreshold: 0.12,
    fadeOutStart: 0.65,
    fadeOutEnd: 0.95,
    translateRange: 36,
  ...
  const imageFade = useTopEdgeFade(scrollY, { fadeStart: 0, fadeEnd: -400, translateMax: 6 });
      ...
      >
        <div className="max-w-6xl xl:min-w-[1400px] mx-auto">
          <div className="grid md:grid-cols-[auto_1fr] gap-10 md:gap-14 items-center">
            ...
            {/* Portrait — order 1 on desktop, reveals with slight scale */}
            <div
              ref={imageFade.ref("portrait")}
              className={`reveal-child${isVisible ? " is-visible" : ""} md:order-1`}
              style={{ "--reveal-delay": "0ms", ...imageFade.style("portrait") } as React.CSSProperties}
            >
              <div className="relative aspect-[3/4] max-w-[400px] overflow-hidden rounded-lg shadow-2xl">
                <Image
            ...
            {/* Text block — order 2 on desktop, staggered after portrait */}
            <div className="flex flex-col gap-6 md:order-2">
              ...
              <p
                ref={topFade.ref("p1")}
                className={`reveal-child${isVisible ? " is-visible" : ""} xl:max-w-xl text-lg md:text-xl leading-relaxed font-light text-pretty`}
                style={{
